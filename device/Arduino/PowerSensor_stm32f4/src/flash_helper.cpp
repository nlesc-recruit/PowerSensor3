...
{
  FLASH_BASE->ACR |= FLASH_ACR_ICRST;
}
...
{
  FLASH_BASE->ACR |= FLASH_ACR_DCRST;
}
...
void FLASH_Unlock(void)
{
  if((FLASH_BASE->CR & FLASH_CR_LOCK) != RESET)
  {
...
FLASH_Status FLASH_EraseSector(uint32_t FLASH_Sector, uint8_t VoltageRange)
{
  ...
  { 
    ...
    FLASH_BASE->CR &= CR_PSIZE_MASK;
    FLASH_BASE->CR |= tmp_psize;
    FLASH_BASE->CR &= SECTOR_MASK;
    FLASH_BASE->CR |= FLASH_CR_SER | FLASH_Sector;
...
FLASH_Status FLASH_ProgramDoubleWord(uint32_t Address, uint64_t Data)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    FLASH_BASE->CR &= CR_PSIZE_MASK;
    FLASH_BASE->CR |= FLASH_PSIZE_DOUBLE_WORD;
...
FLASH_Status FLASH_ProgramWord(uint32_t Address, uint32_t Data)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    /* if the previous operation is completed, proceed to program the new data */
    FLASH_BASE->CR &= CR_PSIZE_MASK;
    FLASH_BASE->CR |= FLASH_PSIZE_WORD;
    FLASH_BASE->CR |= FLASH_CR_PG;
...
FLASH_Status FLASH_ProgramHalfWord(uint32_t Address, uint16_t Data)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    /* if the previous operation is completed, proceed to program the new data */
    FLASH_BASE->CR &= CR_PSIZE_MASK;
    FLASH_BASE->CR |= FLASH_PSIZE_HALF_WORD;
    FLASH_BASE->CR |= FLASH_CR_PG;
...
FLASH_Status FLASH_ProgramByte(uint32_t Address, uint8_t Data)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    /* if the previous operation is completed, proceed to program the new data */
    FLASH_BASE->CR &= CR_PSIZE_MASK;
    FLASH_BASE->CR |= FLASH_PSIZE_BYTE;
    FLASH_BASE->CR |= FLASH_CR_PG;
...
void FLASH_OB_Unlock(void)
{
  if((FLASH_BASE->OPTCR & FLASH_OPTCR_OPTLOCK) != RESET)
  {
    /* Authorizes the Option Byte register programming */
    FLASH_BASE->OPTKEYR = FLASH_OPT_KEY1;
    FLASH_BASE->OPTKEYR = FLASH_OPT_KEY2;
  }  
}
...
void FLASH_OB_Lock(void)
{
  ...
  FLASH_BASE->OPTCR |= FLASH_OPTCR_OPTLOCK;
}
...
FLASH_Status FLASH_OB_Launch(void)
{
  ...
  *(__IO uint8_t *)OPTCR_BYTE0_ADDRESS |= FLASH_OPTCR_STRT;
...
uint8_t FLASH_OB_GetUser(void)
{
  ...
  return (uint8_t)(FLASH_BASE->OPTCR >> 5);
}
...
void FLASH_ITConfig(uint32_t FLASH_IT, FunctionalState NewState)
{
  ...
  if(NewState != DISABLE)
  {
    ...
    FLASH_BASE->CR |= FLASH_IT;
  }
  else
  {
    /* Disable the interrupt sources */
    FLASH_BASE->CR &= ~(uint32_t)FLASH_IT;
  }
}
...
FlagStatus FLASH_GetFlagStatus(uint32_t FLASH_FLAG)
{
  ...
  if((FLASH_BASE->SR & FLASH_FLAG) != (uint32_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
FLASH_Status FLASH_GetStatus(void)
{
  ...
  if((FLASH_BASE->SR & FLASH_FLAG_BSY) == FLASH_FLAG_BSY) 
  {
    flashstatus = FLASH_BUSY;
  }
  else 
  {  
    if((FLASH_BASE->SR & FLASH_FLAG_WRPERR) != (uint32_t)0x00)
    { 
      flashstatus = FLASH_ERROR_WRP;
    }
    else
    {
      if((FLASH_BASE->SR & FLASH_FLAG_RDERR) != (uint32_t)0x00)
      { 
        flashstatus = FLASH_ERROR_RD;
      } 
      else 
      {
        if((FLASH_BASE->SR & (uint32_t)0xE0) != (uint32_t)0x00)
        {
          flashstatus = FLASH_ERROR_PROGRAM; 
        }
        else
        {
          if((FLASH_BASE->SR & FLASH_FLAG_OPERR) != (uint32_t)0x00)
          {
