...
#define MAX_LEADS 12  // 4 current sensors with one lead each and 4 voltage sensors with two leads each
...
uint8_t numLead;  // number of active leads
int activeSensors[MAX_SENSORS]; // which sensors are active
int activeLeads[MAX_LEADS];
uint16_t dmaBuffer[MAX_LEADS];  // 16b per lead
uint16_t avgBuffer[MAX_LEADS][numSampleToAverage];
...
void getActiveSensors() {
  ...
  for (int i=0; i < MAX_SENSORS; i++) {
    if (eeprom.sensors[i].inUse) {
      ...
      numSensor++;
      // i/2 is the number of voltage sensors in the array so far, i.e. the
      // number of extra leads in addition to the sensor index
      activeLeads[numLead] = i + i / 2;
      numLead++;
      if ((i % 2) != 0) {
        // voltage sensor -> has another lead with index one higher than the previous one
        activeLeads[numLead] = activeLeads[numLead - 1] + 1;
        numLead++;
      }
    }
  }
...
void configureADC() {
  ...
  ADCConfig.SequencersScanMode = ADC_SCANMODES[numLead - 1] == LL_ADC_REG_SEQ_SCAN_DISABLE ? LL_ADC_SEQ_SCAN_DISABLE: LL_ADC_SEQ_SCAN_ENABLE;  // enable scan only if there is more than one rank to convert
...
void configureADCChannels() {
  ...
  ADCChannelConfig.SequencerLength = ADC_SCANMODES[numLead - 1];  // number of ranks to convert
  ...
  for (uint8_t i = 0; i < numLead; i++) {
    uint8_t lead_id = activeLeads[i];
    LL_ADC_REG_SetSequencerRanks(ADC1, ADC_RANKS[i], ADC_CHANNELS[lead_id]);
    LL_ADC_SetChannelSamplingTime(ADC1, ADC_CHANNELS[lead_id], LL_ADC_SAMPLINGTIME_3CYCLES);
  }
...
void configureGPIO() {
  ...
  uint32_t pinsA = 0, pinsB = 0, pinsC = 0;
  for (uint8_t i = 0; i < numLead; i++) {
    uint8_t lead_id = activeLeads[i];
    // ADC channels 0-7 are on GPIOA
    // channels 8-9 on GPIOB
    // channels 10-11 on GPIOC (not implemented on BlackPill F401)
    if (lead_id < 8)
      pinsA |= GPIO_PINS[lead_id];
    else if (lead_id < 10)
      pinsB |= GPIO_PINS[lead_id];
    else
      pinsC |= GPIO_PINS[lead_id];
  }

  GPIOConfig.Pin = pinsA;
  if (LL_GPIO_Init(GPIOA, &GPIOConfig) != SUCCESS) {
    Blink(4);
    exit(1);
  }

  GPIOConfig.Pin = pinsB;
  if (LL_GPIO_Init(GPIOB, &GPIOConfig) != SUCCESS) {
    Blink(4);
    exit(1);
  }

// Not implemented on BlackPill F401
  GPIOConfig.Pin = pinsC;
  if (LL_GPIO_Init(GPIOC, &GPIOConfig) != SUCCESS) {
    Blink(4);
    exit(1);
  }
}
...
uint16_t getAverage(uint8_t lead_id) {
  uint16_t level = 0;
  for (int i = 0; i < numSampleToAverage; i++) {
    level += avgBuffer[lead_id][i];
  }
  return level / numSampleToAverage;
}
...
void sendADCValues() {
  ...
  for (uint8_t i = 0; i < numSensor; i++) {
    ...
    if ((sensor_id % 2) == 0) {
      // current sensor, one lead
      level = getAverage(activeLeads[i + i / 2]);
    } else {
      // voltage sensor, two leads
      // first lead is V+, second lead is V-
      level = getAverage(activeLeads[i + i / 2]) - getAverage(activeLeads[i + i / 2 + 1]);
    }

    // turn level into block of 2 bytes with metadata
    // add metadata to remaining bits: 6 bits available with 10b sensor value
